...
class FeeInvoicesRepository implements FeeInvoicesRepositoryInterface
{
    ...
    {
        $Fee_invoices = Fee_invoice::with('fees')->get();
    ...
    public function store($request)
    {
        ...
        try {
            ...
            foreach ($List_Fees as $List_Fee) {
                ...
                $Fees->amount = $List_Fee['amount_paid'];
                $Fees->description = $List_Fee['description'];
                $Fees->save();

                $previousPaidFees = Fee_invoice::where('student_id',$List_Fee['student_id'])->sum('amount');
                $feeToBePaid = Fee::where('id', $List_Fee['fee_id'])->first()->amount;
                $remainingFees = $feeToBePaid - $previousPaidFees;
                ...
                $StudentAccount->Debit = $List_Fee['amount_paid'];
                $StudentAccount->credit = $remainingFees;
    ...
    {
        $fee_inv = Fee_invoice::find($id);
        $student = Student::find($name);
        $fee_type = Fee::find($id);
        return view('pages.Fees_invoices.index', compact('fee_inv', 'student', 'fee_type'));
    }
